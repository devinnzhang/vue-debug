...
gulp.task('bundle:example', function() {
  function standalone(name, entry, exportName) {
    return browserify(entry, { debug: true, standalone: exportName || name })
      .bundle()
      .pipe(fs.createWriteStream('./examples/'+ name.replace(/_/g, '-') +'.js'))
  }
  return standalone('build', './examples/basic_list/script.js')
})

gulp.task('compress:example', ['bundle:example'], function() {
  return gulp.src(['examples/build.js', '!examples/build.min.js'])
    .pipe(sourcemaps.init())
    .pipe(uglify())
    .pipe(rename({ suffix: '.min' }))
    .pipe(sourcemaps.write('.'))
